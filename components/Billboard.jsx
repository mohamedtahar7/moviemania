...
const Billboard = () => {
  ...
  //     )
  //     .then((res) => {
  //       setBillboardMovies(res.data.results);
  //     });
  // }, []);
  const billboardMovies = [
    ...
    },
    {
      ...
      popularity: 2127.199,
      poster_path: "/uOI29oP3wShBLfxtyBD9faPyYqD.jpg",
  ...
  const prevSlide = () => {
    ...
    const newIndex = isFirstSlide
      ? billboardMovies.length - 1
      : currentIndex - 1;
    setCurrentIndex(newIndex);
  };
  const nextSlide = () => {
    const isLastSlide = currentIndex === billboardMovies.length - 1;
  ...
  return (
    <div className="relative group">
      <img
        src={`https://www.themoviedb.org/t/p/w1280_and_h720_face/${billboardMovies[currentIndex].backdrop_path}`}
        alt="backdrop"
        className="w-full"
      />
      <div className="absolute text-7xl bg-black w-full h-full opacity-60 top-0 left-0"></div>
      <BillboardMovie movie={billboardMovies[currentIndex]} />
      <div className="hidden lg:hidden absolute top-[50%] left-5 -translate-x-0 -translate-y-[-50%] text-white group-hover:block md:block hover:opacity-90 transition cursor-pointer">
        <BsArrowLeftCircleFill onClick={prevSlide} size={30} />
      </div>
      <div className="lg:hidden text-white absolute top-[50%] right-5 -translate-x-0 -translate-y-[-50%] group-hover:block md:block hover:opacity-90 transition cursor-pointer">
